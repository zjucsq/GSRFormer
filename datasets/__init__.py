...
from .ov_swig import build as build_ov_swig
...
def build_ov_dataset(image_set, args):
    if args.dataset_file == 'swig':
        return build_ov_swig(image_set, args)
    raise ValueError(f'dataset {args.dataset_file} not supported')

def build_processed_ov_dataset(image_set, args, neighbors_dict=None):
    if args.dataset_file == 'swig':
        return build_ov_swig(image_set, args, processed=True, neighbors_dict=neighbors_dict)
    raise ValueError(f'dataset {args.dataset_file} not supported')