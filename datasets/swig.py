...
class CSVDataset(Dataset):
    ...
    def __init__(self,
        ...
        self.noun_real_name = self._read_nouns(self.imsitu_space_path, self.idx_to_class)
        self.image_data = self._read_annotations(self.SWiG_json, verb_info,
                                                 self.classes)
    ...
    def load_image(self, image_index):
        im0 = Image.open(self.image_names[image_index])
        im = im0.convert('RGB')
        ...
        return im0, im.astype(np.float32) / 255.0
...
def collater(data):
    ...
    return (util.misc.nested_tensor_from_tensor_list(padded_imgs), [{
        ...
        'img_name':
        im
    } for vi, vri, annot, w, h, s0, s1, sc, im in zip(
        verb_indices, verb_role_indices, annot_padded, widths, heights,
        shift_0, shift_1, scales, img_names)])
...
def build(image_set, args):
    ...
    imsitu_space_path = Path(args.swig_path) / "SWiG_jsons" / "imsitu_space.json"
