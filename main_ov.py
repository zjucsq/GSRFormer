...
from datasets import build_ov_dataset, build_processed_ov_dataset
...
from models import build_ov_encoder_model, build_ov_decoder_model
from pathlib import Path
from models.classifier import load_clip_features
...
def get_args_parser():
    ...
    parser.add_argument('--output_folder', default='',
                        help='folder where to save, empty for no saving')
...
def main(args):
    ...
    if args.dataset_file == "swig":
        from datasets.ov_swig import collater, processed_collater
    else:
        assert False, f"dataset {args.dataset_file} is not supported now"

    # build dataset
    dataset_train = build_ov_dataset(image_set='train', args=args)
    args.num_noun_classes = dataset_train.num_nouns()
    dataset_val = build_ov_dataset(image_set='val', args=args)
    dataset_test = build_ov_dataset(image_set='test', args=args)
    base_verb_id_set = set(
        dataset_train.verb_to_idx[verb_name] for verb_name in dataset_train.base_verb)
    # load clip features
    verb_features = load_clip_features(
        dataset_test=dataset_train, device=device, type='verb')
    noun_features = load_clip_features(
        dataset_test=dataset_train, device=device, type='noun')
    # build Encoder Transformer model
    encoder_model, encoder_criterion = build_ov_encoder_model(
        args, verb_features, noun_features, dataset_train, device)
    encoder_model.to(device)
    encoder_model_without_ddp = encoder_model
    if args.load_saved_encoder == True:
        encoder_checkpoint = torch.load(
            args.encoder_saved_model, map_location='cpu')
        encoder_model.load_state_dict(encoder_checkpoint["encoder_model"])
    if args.distributed:
        encoder_model = torch.nn.parallel.DistributedDataParallel(
            encoder_model, device_ids=[args.gpu])
        encoder_model_without_ddp = encoder_model.module
    encoder_model.requires_grad_(True)
    encoder_model.verb_classifier.requires_grad_(False)
    encoder_model.noun_classifier.requires_grad_(False)
    encoder_model.transformer1.verb_classifier.requires_grad_(False)
    num_encoder_parameters = sum(
    ...
    encoder_param_dicts = [
        {"params": [p for n, p in encoder_model_without_ddp.named_parameters(
    ...
    encoder_optimizer = torch.optim.AdamW(
        encoder_param_dicts, lr=args.lr, weight_decay=args.weight_decay)
    encoder_lr_scheduler = torch.optim.lr_scheduler.StepLR(
        encoder_optimizer, args.lr_drop)

    if args.load_saved_encoder == True:
        encoder_optimizer.load_state_dict(
            encoder_checkpoint["encoder_optimizer"])
        encoder_lr_scheduler.load_state_dict(
            encoder_checkpoint["encoder_lr_scheduler"])
        args.encoder_start_epoch = encoder_checkpoint["encoder_epoch"] + 1

    # build Decoder Transformer Model
    decoder_model, decoder_criterion = build_ov_decoder_model(
        args, verb_features, noun_features, dataset_train, device)
    decoder_model.to(device)
    decoder_model_without_ddp = decoder_model
    if args.load_saved_decoder == True:
        decoder_checkpoint = torch.load(
            args.decoder_saved_model, map_location='cpu')
        decoder_model.load_state_dict(decoder_checkpoint["decoder_model"])
    if args.distributed:
        decoder_model = torch.nn.parallel.DistributedDataParallel(
            decoder_model, device_ids=[args.gpu])
        decoder_model_without_ddp = decoder_model.module
    decoder_model.requires_grad_(True)
    decoder_model.verb_classifier.requires_grad_(False)
    decoder_model.noun_classifier.requires_grad_(False)
    num_decoder_parameters = sum(
        p.numel() for p in decoder_model.parameters() if p.requires_grad)
    print('number of Decoder Transformer parameters:', num_decoder_parameters)
    decoder_param_dicts = [
        {"params": [p for n, p in decoder_model_without_ddp.named_parameters(
        ) if "backbone" not in n and p.requires_grad]},
        {
    ...
    decoder_optimizer = torch.optim.AdamW(
        decoder_param_dicts, lr=args.lr, weight_decay=args.weight_decay)
    decoder_lr_scheduler = torch.optim.lr_scheduler.StepLR(
        decoder_optimizer, args.lr_drop)
    if args.load_saved_decoder == True:
        """
    ...
    if args.preprocess == True:
        preprocess_sampler_train = torch.utils.data.RandomSampler(
            dataset_train)
    ...
    if not args.test and not args.dev:
        batch_sampler_train = torch.utils.data.BatchSampler(
            sampler_train, args.encoder_batch_size, drop_last=True)
    ...
    # For Preprocessing
    if args.preprocess == True:
        batch_preprocess_sampler_train = torch.utils.data.BatchSampler(
            preprocess_sampler_train, args.encoder_batch_size, drop_last=False)
        batch_preprocess_sampler_val = torch.utils.data.BatchSampler(
            preprocess_sampler_val, args.encoder_batch_size, drop_last=False)
    ...
    if args.dev or args.test:
        ...
        decoder_checkpoint = torch.load(
            args.decoder_saved_model, map_location='cpu')
        decoder_model.load_state_dict(decoder_checkpoint["decoder_model"])
        encoder_model.set_base(False)
        decoder_model.set_base(False)
        # build dataset
        if args.dev:
            with open("__storage__/{}/ov_valDict.json".format(args.output_folder)) as val_json:
                val_dict = json.load(val_json)
            processed_dataset_val = build_processed_ov_dataset(
                image_set='val', args=args, neighbors_dict=val_dict)
            if args.distributed:
                sampler_val = DistributedSampler(
                    processed_dataset_val, shuffle=False)
            else:
                sampler_val = torch.utils.data.SequentialSampler(
                    processed_dataset_val)
            data_loader = DataLoader(processed_dataset_val, num_workers=args.num_workers,
                                     drop_last=False, collate_fn=processed_collater,
                                     sampler=sampler_val)
        elif args.test:
            with open("__storage__/{}/ov_testDict.json".format(args.output_folder)) as test_json:
                test_dict = json.load(test_json)
            processed_dataset_test = build_processed_ov_dataset(
                image_set='test', args=args, neighbors_dict=test_dict)
            if args.distributed:
                sampler_test = DistributedSampler(
                    processed_dataset_test, shuffle=False)
            else:
        ...
        test_stats, test_stats_base, test_stats_novel = evaluate_swig(encoder_model, decoder_model, decoder_criterion,
                                                                      data_loader, device, args.output_dir, base_verb_id_set)
        # log_stats = {**{f'test_{k}': v for k, v in test_stats.items()}}
        log_stats = {**{f'test_{k}': v for k, v in test_stats.items()},
                     **{f'test_base_{k}': v for k, v in test_stats_base.items()},
                     **{f'test_novel_{k}': v for k, v in test_stats_novel.items()}}
        ...
        if args.dev:
            write_log(output_dir, log_stats, "best", "dev")
        elif args.test:
            write_log(output_dir, log_stats, "best", "test")

        return None

    if not os.path.exists(f"__storage__/{args.output_folder}"):
        os.mkdir(f"__storage__/{args.output_folder}")
    # train GSRFormer Encoder Transformer
    print("Start training GSRFormer Encoder Transformer at epoch ",
          args.encoder_start_epoch)
    ...
    for epoch in range(args.encoder_start_epoch, args.encoder_epochs):
        ...
        # evaluate
        encoder_model.set_base(True)
        test_stats, test_stats_base, test_stats_novel = encoder_evaluate_swig(
            encoder_model, encoder_criterion, data_loader_val, device, args.output_dir, base_verb_id_set)
        ...
        if args.output_dir:
            ...
            if log_stats['test_verb_acc_top1_unscaled'] > max_test_verb_acc_top1:
                ...
                checkpoint_paths.append(
                    output_dir / f'encoder_checkpoint{epoch:04}.pth')
        ...
        if args.output_dir and utils.is_main_process():
            with (output_dir / "log.txt").open("a") as f:
                f.write(json.dumps(log_stats) + "\n")
        write_log(output_dir, log_stats, epoch, "encoder")
    ...
    if args.preprocess == True:
        preprocess_neighbors(encoder_model, preprocess_data_loader_train,
                             "train", device, args.images_per_segment, args.output_folder, True)
        preprocess_neighbors(encoder_model, preprocess_data_loader_val,
                             "val", device, args.images_per_eval_segment, args.output_folder, True)
        preprocess_neighbors(encoder_model, preprocess_data_loader_test,
                             "test", device, args.images_per_eval_segment, args.output_folder, True)
    if args.decoder_start_epoch >= args.decoder_epochs:
        return None

    # build Decoder Transformer dataset
    with open("__storage__/{}/ov_trainDict.json".format(args.output_folder)) as train_json:
        train_dict = json.load(train_json)
    with open("__storage__/{}/ov_valDict.json".format(args.output_folder)) as val_json:
        val_dict = json.load(val_json)
    processed_dataset_train = build_processed_ov_dataset(
        "train", args, neighbors_dict=train_dict)
    processed_dataset_val = build_processed_ov_dataset(
        "val", args, neighbors_dict=val_dict)
    ...
    batch_sampler_train = torch.utils.data.BatchSampler(
        sampler_train, args.decoder_batch_size, drop_last=True)
    ...
    print("Start training GSRFormer Decoder Transformer at epoch ",
          args.decoder_start_epoch)
    start_time = time.time()
    max_test_verb_acc_top1 = 43
    for epoch in range(args.decoder_start_epoch, args.decoder_epochs):
        # train one epoch
        encoder_model.set_base(True)
        decoder_model.set_base(True)
        if args.distributed:
            sampler_train.set_epoch(epoch)
        ...
        decoder_model.set_base(True)
        test_stats, test_stats_base, test_stats_novel = evaluate_swig(encoder_model, decoder_model, decoder_criterion,
                                                                      data_loader_val, device, args.output_dir, base_verb_id_set)
        ...
        if args.output_dir:
            ...
            if log_stats['test_verb_acc_top1_unscaled'] > max_test_verb_acc_top1:
                max_test_verb_acc_top1 = log_stats['test_verb_acc_top1_unscaled']
                checkpoint_paths.append(
                    output_dir / f'decoder_checkpoint{epoch:04}.pth')
            for checkpoint_path in checkpoint_paths:
        ...
        if args.output_dir and utils.is_main_process():
            with (output_dir / "log.txt").open("a") as f:
                f.write(json.dumps(log_stats) + "\n")
        write_log(output_dir, log_stats, epoch, "decoder")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        'GSRFormer training and evaluation script', parents=[get_args_parser()])
