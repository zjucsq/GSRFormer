...
def main(args):
    ...
    if args.load_saved_encoder == True:
        encoder_checkpoint = torch.load(args.encoder_saved_model,
                                        map_location='cpu')
    ...
    encoder_param_dicts = [{
        "params": [
            p for n, p in encoder_model_without_ddp.named_parameters()
    ...
    encoder_optimizer = torch.optim.AdamW(encoder_param_dicts,
                                          lr=args.lr,
    ...
    if args.load_saved_decoder == True:
        decoder_checkpoint = torch.load(args.decoder_saved_model,
                                        map_location='cpu')
    ...
    decoder_param_dicts = [{
        "params": [
            p for n, p in decoder_model_without_ddp.named_parameters()
            if "backbone" not in n and p.requires_grad
        ]
    }, {
        "params": [
    ...
    decoder_optimizer = torch.optim.AdamW(decoder_param_dicts,
                                          lr=args.lr,
                                          weight_decay=args.weight_decay)
    ...
    if args.preprocess == True:
        ...
        preprocess_data_loader_train = DataLoader(
            dataset_train,
            num_workers=args.num_workers,
            drop_last=False,
            collate_fn=collater,
            batch_sampler=batch_preprocess_sampler_train)
        preprocess_data_loader_val = DataLoader(
            dataset_val,
            num_workers=args.num_workers,
            drop_last=False,
            collate_fn=collater,
            batch_sampler=batch_preprocess_sampler_val)
    ...
    if args.dev or args.test:
        ...
        if args.dev:
            ...
            processed_dataset_val = build_processed_dataset(
                image_set='val', args=args, neighbors_dict=val_dict)
            if args.distributed:
                sampler_val = DistributedSampler(processed_dataset_val,
                                                 shuffle=False)
            else:
                sampler_val = torch.utils.data.SequentialSampler(
                    processed_dataset_val)
            data_loader = DataLoader(processed_dataset_val,
                                     num_workers=args.num_workers,
                                     drop_last=False,
                                     collate_fn=collater,
                                     sampler=sampler_val)
        ...
        test_stats = evaluate_swig_zero_shot(
                                   decoder_criterion, data_loader, device,
                                   args.output_dir)
